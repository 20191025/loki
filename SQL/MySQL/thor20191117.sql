CREATE DATABASE  IF NOT EXISTS `thor` /*!40100 DEFAULT CHARACTER SET latin1 */ /*!80016 DEFAULT ENCRYPTION='N' */;
USE `thor`;
...
CREATE TABLE `COTIZACIONES` (
  ...
  `fechaCreacion` datetime NOT NULL,
  `codCotizacion` varchar(17) NOT NULL,
...
--
/*!50003 DROP FUNCTION IF EXISTS `fn_numeroRegistro` */;
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `fn_numeroRegistro`( tipReg varchar(3) ) RETURNS varchar(20) CHARSET latin1
    DETERMINISTIC
BEGIN

   DECLARE numeroRegistro VARCHAR(17);

   SELECT concat(ifnull(tipReg,""),date_format(current_timestamp(),"%Y%m%d%k%i%s")) INTO numeroRegistro;
...
-- Dump completed on 2019-11-18  6:25:40
